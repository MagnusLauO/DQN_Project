...
from BallsOneHot import BallsGame
import time
import os
import csv
...
Gradient_update = 10
State_dim = 11  # Modified for new state space
Hidden_dim = 128
Action_dim = 3  # Modified for new action space
Number = 0
Max_steps = 5000 # To shorten train time or else the agent gets too good.
...
def train():
    ...
    agent_dir = f"Gus1_Agents/agent_OneHot_{str(Current_Network)}_{Hidden_dim}_{Number}_{Episodes}_{time.strftime('%Y-%m-%d_%H-%M-%S')}" # Directory with date and time in folder called Agents
    ...
    for i in range(Episodes):
        ...
        state = state_to_input1(start_state).to(device) # convert to input
        ...
        while (not done) and (episode_step < Max_steps):
            ...
            new_state = state_to_input1(new_state).to(device)
            GameReward += reward
    ...
    # Save model
    torch.save(NN.state_dict(), os.path.join(agent_dir, 'ballsGame_q_net.pt'))
