...
Gradient_update = 10
State_dim = 50  # Modified for new state space
Hidden_dim = 128
...
def train():
    ...
    agent_dir = f"Gus1_Agents/agent_Grid_{str(Current_Network)}_{Hidden_dim}_{Number}_{Episodes}_{time.strftime('%Y-%m-%d_%H-%M-%S')}" # Directory with date and time in folder called Agents
    ...
    for i in range(Episodes):
        ...
        state = state_to_input2(start_state).to(device) # convert to input
        ...
        while (not done) and (episode_step < Max_steps):
            ...
            new_state = state_to_input2(new_state).to(device)
