...
class BallsGame:
    ...
    ## Get state ##
    def get_state(self):
        # Initialiser state med alle værdier som 0
        state = np.zeros((self.rows, self.cols), dtype=int)
        
        # Marker spilleren som 1
        state[self.rows - 1, self.player_x] = 1
        
        # Marker boldene som -1
        for ball in self.balls:
            if 0 <= int(ball.y) < self.rows:  # Bold skal være inden for grid
                state[int(ball.y), int(ball.x)] = -1
...
class Ball:
    ...
    # move
    def move(self, speed: float, boost: float):
        self.y += speed + boost
